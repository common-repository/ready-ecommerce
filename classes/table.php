...
abstract class table {
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
		...
		$data = db::get($query, $return);
		if($this->_prepareHtml) {
			$data = db::prepareHtmlOut($data);
		}
    ...
    public function prepareInput($d = array()) {
        ...
        foreach($this->_fields as $key => $f) {
            if($f->type == 'tinyint') {
                if(isset($d[$key]) && $d[$key] == 'true')
                    $d[$key] = 1;
                if((!isset($d[$key]) || empty($d[$key])) && !in_array($key, $ignore)) {
                    $d[$key] = 0;
                }
            }
            if($f->type == 'date') {
                if((!isset($d[$key]) || empty($d[$key])) && !in_array($key, $ignore)) {
                    $d[$key] = '0000-00-00';
                } elseif(isset($d[$key]) && !empty($d[$key])) {
	...
	/**
	 * Run on ex. module install
	 */
    public function install($d = array()) {
        
    }
	/**
	 * Run on ex. module uninstall
	 */
    public function uninstall($d = array()) {
        
    }
	/**
	 * Run on ex. module activated
	 */
