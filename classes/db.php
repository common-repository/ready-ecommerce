...
class db {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see S_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, S_DB_PREF, $wpdb->prefix. S_DB_PREF),
                $query);
    }
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, S_DATE_DL)) return dateToTimestamp($date);
        $arr = explode('-', $date);
        return dateToTimestamp($arr[2]. S_DATE_DL. $arr[1]. S_DATE_DL. $arr[0]);
    }
