...
<div class="container">
	<form id="toeWelcomePageFindUsForm" method="GET">
		<h1>
			<?php lang::_e('Welcome to')?>
			<?php echo S_WP_PLUGIN_NAME?>
			<?php lang::_e('Version')?>
			<?php echo S_VERSION?>!
		</h1>
		<div class="clear"></div>
		<div class="about-message">
			This is first start up of the <?php echo S_WP_PLUGIN_NAME?> plugin.<br />
		...
		<div class="plug-icon-shell">
			...
			<a target="_blank" href="http://readyshoppingcart.com/"><?php echo S_WP_PLUGIN_NAME?></a><br />
			<a target="_blank" href="http://readyshoppingcart.com/"><?php echo S_VERSION?></a><br />
		</div>
		<div class="clear"></div>
		...
		<?php foreach($this->askOptions as $askId => $askOpt) { ?>
			<label><?php echo html::radiobutton('where_find_us', array('value' => $askId))?>&nbsp;<?php echo $askOpt['label']?></label>
			<?php if($askId == 4 /*Find on the web*/) { ?>
				<label id="toeFindUsUrlShell" style="display: none;">Please, post url: <?php echo html::text('find_on_web_url')?></label>
			<?php } elseif($askId == 5 /*Other way*/) { ?>
				<label style="display: none;" id="toeOtherWayTextShell"><?php echo html::textarea('other_way_desc')?></label>
			<?php }?>
		<?php }?>
		...
		<?php echo html::hidden('page', array('value' => 'promo_ready'))?>
		<?php echo html::hidden('action', array('value' => 'welcomePageSaveInfo'))?>
		<?php echo html::submit('gonext', array('value' => 'Thank for check info. Start using plugin.', 'attrs' => 'class="button button-primary button-hero"'))?>
		<?php echo html::hidden('original_page', array('value' => req::getVar('page')))?>
